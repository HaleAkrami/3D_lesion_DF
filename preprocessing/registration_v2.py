...
def main(args=None):
    ...
    if  os.path.isdir(os.path.join(Path(args.img_dir).parents[0],'t2')) and args.nomaskandseg !=1:
        t2_path = os.path.join(Path(args.img_dir).parents[0],'t2')
        t2_out = os.path.join(Path(args.out_dir).parents[0],'t2')
        Path(t2_out).mkdir(parents=True,exist_ok=True)
    else: 
        t2_path = None
    ...
    for i, file in tqdm(enumerate(os.listdir(args.img_dir))):
        if not os.path.isfile(os.path.join(dest_basepath, file)) or not os.path.isfile(os.path.join(mask_out, file.replace(args.modality,'_mask'))) or not os.path.isfile(os.path.join(seg_out, file.replace(args.modality,'_seg'))):
            ...
            if t2_path is not None:
                path_t2 = os.path.join(t2_path, file.replace(args.modality,'_t2'))
                moving_t2 = ants.image_read(path_t2)
                moving_t2 = moving_t2.reorient_image2('RAI') # reorient to standard orientation
                # transform the segmentation in the same way as the volume
                moved_t2 = ants.apply_transforms( fixed=fixed_im, moving=moving_t2,
                                           transformlist=im_tx['fwdtransforms'] )
                ants.image_write(moved_t2, os.path.join(t2_out, file.replace(args.modality,'_t2')))
