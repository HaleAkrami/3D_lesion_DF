...
model_path = '/acmenas/hakrami/3D_lesion_DF/models/test/model_epoch149.pt'
...
for epoch in range(n_epochs):
    ...
    if (epoch + 1) % val_interval == 0:
        ...
        plt.imshow(image[0, 0, :, :, 80].cpu(), vmin=0, vmax=1, cmap="gray")
        plt.tight_layout()
        plt.axis("off")
        plt.show()
        # Modify the filename to include the epoch number
        filename = f"./results/test/sample_epoch{epoch+epoch_prev}.png"

        plt.savefig(filename, dpi=300)  
        # Save the model
        model_filename = f"./models/test/model_epoch{epoch+epoch_prev}.pt"
