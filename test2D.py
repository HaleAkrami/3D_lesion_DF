...
import wandb
wandb.init(project='2D_ddpm',name='test')
...
ssim_loss = MaskedLoss(SSIMLoss,spatial_dims=2, data_range=1)
...
for step, batch in tqdm.tqdm(enumerate(test_loader), total=len(test_loader)):
    ...
    print(data_range)
    ssim_loss = MaskedLoss(SSIMLoss,spatial_dims=2, data_range=data_range)
    ...
    gt_segmentation = (batch['seg']['data']>0).float().to(device).permute(4,0,1, 3, 2)[:,0,:,:,:]
    ...
    labels_flat = gt_segmentation.reshape(-1).cpu().numpy()

    scores_flat = error.reshape(-1).cpu().numpy()
    ...
    if step % 10 ==0:
        ...
        # Segmented Image
        image_array_slice = gt_segmentation[middle_slice_idx][0][:,:].squeeze().cpu().numpy()
        ...
        axes[1, 1].imshow(thresholded_error[middle_slice_idx][0][:,:].squeeze().cpu().numpy(), vmin=0, vmax=1, cmap='gray')
        axes[1, 1].set_title('Thresholded Difference Image')

        axes[1, 2].imshow(error[middle_slice_idx][0][:,:].squeeze().cpu().numpy(), vmin=0, vmax=1, cmap='gray')
        axes[1, 1].set_title('error image')
        ...
        #plt.savefig(fig_filename)  # Save the figure
        plt.show()
        wandb.log({"sample_image_lesion": [wandb.Image(plt)]})
...
print(f"PR-AUC the test set: {pr_auc}")
# Average Dice score
